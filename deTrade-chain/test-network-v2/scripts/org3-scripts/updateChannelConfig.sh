...
setGlobals 2
set -x
peer channel update -f ${TEST_NETWORK_HOME}/channel-artifacts/org3_update_in_envelope.pb -c ${CHANNEL_NAME} -o localhost:7050 --ordererTLSHostnameOverride orderer0.orderer.example.com --tls --cafile "$ORDERER_CA"
{ set +x; } 2>/dev/null
