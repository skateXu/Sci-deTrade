...
setGlobals() {
  ...
  if [ $USING_ORG -ge 1 ] && [ $USING_ORG -le 20 ]; then
    ...
    export CORE_PEER_ADDRESS=localhost:${PEER_PORTS["${USING_ORG},${USING_PEER}"]}

    # Set the appropriate CA  dynamically
    export PEER${USING_PEER}_ORG${USING_ORG}_CA="${TEST_NETWORK_HOME}/organizations/peerOrganizations/org${USING_ORG}.example.com/tlsca/tlsca.org${USING_ORG}.example.com-cert.pem"
...
# parsePeerConnectionParameters ORG PEER [ORG PEER...]
parsePeerConnectionParameters() {
  PEER_CONN_PARMS=()
  PEERS=""
  while [ "$#" -gt 0 ]; do
